...
* del primer pendulo
*/
function dThetaFA1 (pendulumA, pendulumB, t, kTheta1, kTheta2, kOmega1, kOmega2) {
  return pendulumA.angularVelocity+kOmega1;
}

function dThetaFA2 (pendulumA, pendulumB, t, kTheta1, kTheta2, kOmega1, kOmega2) {
  return pendulumB.angularVelocity+kOmega2;
}

function dOmegaFA1 (pendulumA, pendulumB, t, kTheta1, kTheta2, kOmega1, kOmega2) {
  ...
  var dOmega = -((l1 ** 2 * l2 * m2 * omega1 ** 2 +
    ...
    m2 * (theta2) + b * l1 * omega2) + (-g *
    l1 * l2 * m2 - g * l1 * l2 * m1) * (theta1) - b * l2 * omega1) /
...
function dOmegaFA2 (pendulumA, pendulumB, t, kTheta1, kTheta2, kOmega1, kOmega2) {
  var m1 = pendulumA.mass;
  var m2 = pendulumB.mass;
  var l1 = pendulumA.length;
  var l2 = pendulumB.length;
  ...
  var dOmega = ((l1 * l2 ** 2 * m2 ** 2 * omega2 ** 2 +
    ...
    m2) * (theta1) - b * l2 * m2 * omega1) + g *
    l1 * l2 * m2 ** 2 * (theta2) + g * l1 * l2 * m1 * m2 *
    (theta2) + (b * l1 * m2 + b * l1 * m1) * omega2) / (l1 * l2 ** 2 *
