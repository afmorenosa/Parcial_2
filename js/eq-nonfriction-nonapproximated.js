...
* del primer pendulo
*/
function dThetaNN1 (pendulumA, pendulumB, t, kTheta1, kTheta2, kOmega1, kOmega2) {
  return pendulumA.angularVelocity+kOmega1;
}

function dThetaNN2 (pendulumA, pendulumB, t, kTheta1, kTheta2, kOmega1, kOmega2) {
  return pendulumB.angularVelocity+kOmega2;
}

function dOmegaNN1 (pendulumA, pendulumB, t, kTheta1, kTheta2, kOmega1, kOmega2) {
  ...
  var dOmega = -((l1 ** 2 * l2 * m2 * omega1 ** 2 * Math.cos(theta2 - theta1) +
    ...
    l2 * m2 * Math.sin(theta2)) * Math.cos(theta2 - theta1) + (-g * l1 * l2 *
    m2 - g * l1 * l2 * m1) * Math.sin(theta1)) / (l1 ** 2 * l2 * m2 *
    Math.cos(theta2 - theta1) ** 2 - l1 ** 2 * l2 * m2 - l1 ** 2 * l2 * m1);

  return dOmega;
}

function dOmegaNN2 (pendulumA, pendulumB, t, kTheta1, kTheta2, kOmega1, kOmega2) {
  var m1 = pendulumA.mass;
  var m2 = pendulumB.mass;
  var l1 = pendulumA.length;
  var l2 = pendulumB.length;
  ...
  var dOmega = ((l1 * l2 ** 2 * m2 ** 2 * omega2 ** 2 * Math.cos(theta2 - theta1)
    ...
    m2) * Math.sin(theta1)) * Math.cos(theta2 - theta1) + g * l1 * l2 * m2 **
    2 * Math.sin(theta2) + g * l1 * l2 * m1 * m2 * Math.sin(theta2))
     / (l1 * l2 ** 2 * m2 ** 2 * Math.cos(theta2 - theta1) ** 2 -
    l1 * l2 ** 2 * m2 ** 2 - l1 * l2 ** 2 * m1 * m2);
